...
if($billtype == "Prescription charge")
{
	...
	$rstreatment = mysqli_fetch_array($qsqltreatment);
	//Prescription charge
	$sql ="INSERT INTO billing_records( billingid, bill_type_id, bill_type, bill_amount, bill_date, status) VALUES ('$billid','$prescriptionid','Prescription Charge','$billamt','$dt','Active')";
	$qsql=mysqli_query($con,$sql);
}

if($billtype == "Prescription update")
{
	$sqlprescription_records = "SELECT sum(cost) FROM prescription_records WHERE prescription_id='$_GET[prescriptionid]'";
